...
#            print '\nparts2', self.partuid
    ...
    @debugdecorator
    def AppendRows(self, c, numRows=1, data=np.nan):
        part = self._OM.new('part', c)
        part.defaultdata = data
...
class PartEntryDialog(wx.Dialog):
    def __init__(self, *args, **kwargs):
        if 'size' not in kwargs:
            kwargs['size'] = (240, 120)
        super(PartEntryDialog, self).__init__(*args, **kwargs)

        fgs = wx.FlexGridSizer(2, 2, 3, 3)
#        name_label = wx.StaticText(self, label="Nome: ")
#        unit_label = wx.StaticText(self, label="Unit: ")
        topo_label = wx.StaticText(self, label="Topo: ")
        base_label = wx.StaticText(self, label="Base: ")
#        self.name_ctrl = wx.TextCtrl(self)
#        self.unit_ctrl = wx.TextCtrl(self)
        self.topo_ctrl = wx.TextCtrl(self)
        self.base_ctrl = wx.TextCtrl(self)
        fgs.AddMany(#[(name_label), (self.name_ctrl, 1, wx.EXPAND),
#                     (unit_label), (self.unit_ctrl, 1, wx.EXPAND)])
                     [(topo_label), (self.topo_ctrl, 1, wx.EXPAND),
                     (base_label), (self.base_ctrl, 1, wx.EXPAND)])
        fgs.AddGrowableCol(1, 1)
...
#    def set_value(self, name, unit):
    def set_value(self, name, topo, base):
#        self.name_ctrl.SetValue(name)
#        self.unit_ctrl.SetValue(unit)
        self.unit_ctrl.SetValue(topo)
        self.unit_ctrl.SetValue(base)

    def get_value(self):
#        name = self.name_ctrl.GetValue()
#        unit = self.unit_ctrl.GetValue()
        topo = self.topo_ctrl.GetValue()
        base = self.base_ctrl.GetValue()
#        return name, unit
        return topo, base
...
#            self.grid.ForceRefresh()
    ...
    def on_add_part(self, event):
        dlg = PartEntryDialog(self)
        if dlg.ShowModal() == wx.ID_OK:
            topo, base = dlg.get_value()
#            print 'kk'
...
#        self.grid.ForceRefresh()
#        a = [self._OM.get(partitionuid) for partitionuid in self.partitionmap]
#        b = [part.uid for part in self._OM.list('part', a[0].uid)]
#        print len(self._OM.list('data_index', self.wellmap[0])[0].data) #self._OM.get(self.wellmap[0]).start
        depth = self._OM.list('data_index', self.wellmap[0])[0].data
        topo = float(topo)
        base = float(base)
        a = depth>topo
        b = depth<base
...
#        print c
        for i in range (len(a)):
            c[i] = a[i] and b[i]
#        for part in self._OM.list('partition'):
#            print part
#        print 'partss', topo, base, c
        self.grid.AppendRows(c[0])
        self.grid.ForceRefresh()
#        dlg = PartEntryDialog(self)
#        if dlg.ShowModal() == wx.ID_OK:
#            name, topo, base = dlg.get_value()
...
##        for part in self._OM.list('partition'):
##            print part
#        print 'partss', self.partitionmap, len(self._OM.get(b[0]).data)
##        dlg = PartEntryDialog(self)
##        if dlg.ShowModal() == wx.ID_OK:
##            name, topo, base = dlg.get_value()    
