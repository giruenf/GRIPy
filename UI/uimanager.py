...
class UIModelBase(UIBase):
    ...
    def _do_set(self, key, value):
        ...
        msg = u'    {} has setted attribute {} = {}'.format(self.uid, unicode(key), unicode(self[key]))
        logging.debug(msg)                
    ...
    def _loadstate(self, **state):
        if not state:
            return
        logging.debug('Loading {} state...'.format(self.__class__.__name__))
        for key, value in state.items():
            if key not in self._SPECIALS_KEYS:     
                ...
                logging.error(msg)
        logging.debug('{} state has loaded.'.format(self.__class__.__name__))
...
class UIViewBase(UIBase):
    ...
    def __init__(self, controller_uid):
        ...
        #if self._controller.parent:
        #    self.parent = self._controller.parent.view    
        #else:
        #    self.parent = None
        _UIM = UIManager()  
        self.oid = _UIM._getnewobjectid(self.tid)
...
class UIManager(object):#GenericObject):
    ...
    def register_class(cls, controller_class, model_class, view_class,
        ...
        if controller_parent_class:
            ...
            logging.info('UIManager registered class {} for parent class {} successfully.'.format(class_full_name, parent_full_name))
        else:    
            logging.info('UIManager registered class {} successfully.'.format(class_full_name))                          
        return True
    ...
    def remove(self, uid):
        ...
        if obj.model:
            msg = 'Deleting UI object {}.'.format(obj.model.uid)
            logging.debug(msg)
            del obj.model
        if obj.view:
            view = obj.view
            ...
            obj.view = None
            msg = 'Deleting UI object {}.'.format(view.uid)
            logging.debug(msg)
            del view
        ...
        # And from _data
        del self._data[uid]  
        msg = 'Deleting UI object {}.'.format(uid)
        logging.debug(msg)
        print '   ', obj.__dict__#, type(obj)
        try:
            if obj.view:
                print obj.view.__dict__
            #del obj.oid # truque teste   
        except AttributeError:
            print
            print obj.__dict__
            pass
    ...
    def close(self):
        logging.info('UI Manager has received a close call.')
        for top_level_uid in self._get_top_level_uids():
            self.remove(top_level_uid)
        logging.info('UI Manager has closed.')
