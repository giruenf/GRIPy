...
from lib.yapsy.compat import ConfigParser, is_py2, basestring

from lib.yapsy.PluginInfo import PluginInfo
from lib.yapsy import PLUGIN_NAME_FORBIDEN_STRING
from lib.yapsy.IPluginLocator import IPluginLocator
    ...
    def _extractCorePluginInfo(self, directory, filename):
        ...
        if not isinstance(filename, basestring):
            ...
            with open(candidate_infofile_path) as candidate_infofile:
                name, moduleName, config_parser = self.getPluginNameAndModuleFromStream(candidate_infofile,
                                                                                        candidate_infofile_path)
                ...
                for filename in item[2]:
                    ...
                    for analyzer in self._analyzers:
                        ...
                        if candidate_infofile in _discovered:
                            log.debug("%s (with strategy %s) rejected because already discovered" % (
                        ...
                        if plugin_info is None:
                            log.debug("Plugin candidate '%s'  rejected by strategy '%s'" % (
                            candidate_infofile, analyzer.name))
                        ...
                        if os.path.isdir(plugin_info.path):
                        ...
                        elif (plugin_info.path.endswith(".py") and os.path.isfile(plugin_info.path)) or os.path.isfile(
                                plugin_info.path + ".py"):
                            ...
                            log.error("Plugin candidate rejected: cannot find the file or directory module for '%s'" % (
                                candidate_infofile))
                            break
